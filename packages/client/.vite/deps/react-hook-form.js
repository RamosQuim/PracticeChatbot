import { i as __toESM, t as require_react } from './react-B9D5fVgU.js';
//#region ../../node_modules/.bun/react-hook-form@7.80.0+b1ab299f0a400331/node_modules/react-hook-form/dist/index.esm.mjs
...
var useIsomorphicLayoutEffect = isWeb
...
var isEmptyObjectWithCustomPrototype = (object, keys) =>
...
function Form(props) {
   ...
   const submit = import_react.useCallback(
      async (event) => {
         let hasError = false;
         let type = '';
...
function createFormControl(props = {}) {
   ...
   const hasExplicitNullIntermediate = (name) => {
      ...
      for (let i = 0; i < segments.length - 1; i++) {
         ...
         formValues = isNullOrUndefined(formValues)
            ? formValues
            : formValues[key];
   ...
   const watch = (name, defaultValue) => {
      if (isFunction(name)) {
         ...
         const { unsubscribe } = _subjects.state.subscribe({
            next: (payload) =>
               'values' in payload &&
               name(payload.values || _getWatch(void 0, defaultValue), payload),
         });
         let called = false;
         return {
            unsubscribe: () => {
   ...
   const _subscribe = (props) => {
      var _a;
      const needsValues = !!((_a = props.formState) === null || _a === void 0
         ? void 0
         : _a.values);
